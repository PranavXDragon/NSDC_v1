...
export default function ProjectsShowcase({ projects }: ProjectsShowcaseProps) {
  ...
  if (!projects || projects.length === 0) {
    return (
      <section className="relative py-32 md:py-48 px-6 md:px-12 lg:px-20 min-h-[80vh] flex items-center bg-[#030303]">
        <div className="max-w-[1400px] mx-auto w-full text-center">
          ...
          >
             <span className="font-mono text-[10px] uppercase tracking-[0.3em] text-nsdc-blue border border-nsdc-blue/30 bg-nsdc-blue/10 px-4 py-1.5 rounded-full">
                01 — PROJECTS IN DEVELOPMENT
             </span>
          </motion.div>
          <motion.h2 
            ...
            className="text-4xl md:text-6xl lg:text-7xl font-black text-[var(--foreground)] mb-6 tracking-normal" 
            style={{ fontFamily: "var(--font-heading)" }}
          >
            BUILDING THE <span className="text-nsdc-blue">FIRST WAVE</span>
          </motion.h2>
          <motion.p 
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            Our members are currently developing projects across AI, data science, web development, automation, and emerging technologies.
          </motion.p>
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            {["AI", "DATA SCIENCE", "WEB", "HARDWARE"].map(tag => (
               <span key={tag} className="border border-white/10 bg-white/5 px-4 py-2 font-mono text-[10px] uppercase tracking-[0.1em] text-white/50 rounded-full">
                 {tag}
               </span>
            ))}
          </motion.div>
          <motion.div 
            ...
            className="inline-block border border-dashed border-white/20 bg-white/5 backdrop-blur-sm px-8 py-4 rounded-xl"
          >
             <span className="font-mono text-xs tracking-widest text-white/60 uppercase">STATUS: COMING SOON</span>
          </motion.div>
        </div>
      </section>
    );
      ...
      <div className="sticky top-0 h-screen overflow-hidden">
        ...
        >
          <div className="flex items-center gap-3 md:gap-4">
            <span className="h-px w-6 bg-nsdc-blue/50 md:w-8" />
...
function ProjectSlide({ project, index }: { project: Project; index: number }) {
  return (
    <div className="relative flex h-full w-screen shrink-0 items-center justify-center px-4 pt-16 pb-20 md:px-16 md:py-0 lg:px-24">
      {/* Background tint */}
      <div className="absolute inset-0 bg-nsdc-blue/5" />
      ...
      <div className="relative z-10 grid w-full max-w-7xl gap-6 md:gap-8 lg:grid-cols-2 lg:gap-16">
        ...
        >
          ...
          <div className="absolute -inset-4 rounded-2xl bg-nsdc-blue/10 blur-2xl md:-inset-8 md:rounded-3xl md:blur-3xl" />
        ...
        <div className="flex flex-col justify-center order-2 lg:order-1">
          ...
          <motion.p
            ...
            className="mt-2 text-sm text-nsdc-blue md:mt-3 md:text-xl"
          >
            {project.subtitle}
          </motion.p>
