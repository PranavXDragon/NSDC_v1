...
function parseContent(content: string) {
  ...
  const processInlineFormatting = (text: string): React.ReactNode => {
    ...
    return parts.map((part, i) => {
      if (part.startsWith('`') && part.endsWith('`')) {
        return (
          <code key={i} className="px-1.5 py-0.5 bg-white/10 rounded text-nsdc-blue font-mono text-sm">
            {part.slice(1, -1)}
          </code>
        );
      }
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith('### ')) {
      ...
      flushParagraph();
      elements.push(
        <h2 key={key++} className="mt-12 mb-6 text-2xl md:text-3xl font-bold text-[var(--foreground)] border-l-2 border-nsdc-blue pl-4">
          {trimmed.replace('## ', '')}
        </h2>
      );
    } else if (trimmed.startsWith('# ')) {
    ...
    // Check for numbered lists
      ...
      elements.push(
        <div key={key++} className="ml-4 md:ml-6 mb-3 flex gap-3 text-white/70">
          <span className="text-nsdc-blue font-mono text-sm flex-shrink-0">{trimmed.match(/^\d+/)?.[0]}.</span>
    ...
    // Check for bullet lists
      ...
      elements.push(
        <div key={key++} className="ml-4 md:ml-6 mb-3 flex gap-3 text-white/70">
          <span className="text-nsdc-blue mt-2">•</span>
    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          <motion.div
            className="fixed top-0 left-0 right-0 h-1 bg-nsdc-blue/20 z-[110]"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ delay: 0.3 }}
          >
            <motion.div
              className="h-full bg-nsdc-blue"
          ...
          >
            <div className="max-w-7xl mx-auto px-4 md:px-8 h-16 md:h-20 flex items-center justify-between">
              ...
              >
                <div className="w-10 h-10 rounded-full border border-white/10 flex items-center justify-center group-hover:border-nsdc-blue/50 group-hover:bg-nsdc-blue/10 transition-all">
                  <ArrowLeft className="w-4 h-4 group-hover:text-nsdc-blue transition-colors" />
                </div>
              ...
              <div className="absolute left-1/2 -translate-x-1/2 hidden md:block">
                <span className="text-xs font-medium tracking-widest text-nsdc-blue uppercase">
                  {post.category}
                </span>
              </div>
              ...
              <div className="flex items-center gap-2">
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  className="w-10 h-10 rounded-full border border-white/10 flex items-center justify-center hover:border-nsdc-blue/50 hover:bg-nsdc-blue/10 transition-all"
                >
          ...
          >
            ...
            >
              ...
              <div className="relative z-10 w-full max-w-4xl mx-auto px-6 md:px-8 pb-12 md:pb-20">
                ...
                >
                  <span className="px-4 py-1.5 bg-nsdc-blue/20 border border-nsdc-blue/30 text-xs font-medium tracking-wider text-nsdc-blue uppercase rounded-full">
                    {post.category}
                  </span>
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-11 h-11 md:w-12 md:h-12 rounded-full bg-gradient-to-br from-nsdc-blue/40 to-nsdc-blue/10 border border-white/10 flex items-center justify-center">
                      <span className="text-sm md:text-base font-bold text-nsdc-blue">
                    ...
                    <div>
                      ...
                      <span className="block text-xs text-nsdc-blue/70">
                        {post.author.role}
                      </span>
                    </div>
                  </div>
            ...
            >
              ...
              <div className="mb-10 flex items-center gap-4">
                ...
                <div className="flex gap-1">
                  {[0, 1, 2].map((i) => (
                    <div key={i} className="w-1.5 h-1.5 rounded-full bg-nsdc-blue/40" />
                  ))}
                </div>
                <div className="h-px flex-1 bg-gradient-to-r from-transparent via-white/10 to-transparent" />
              </div>
              ...
              {/* Opening Quote */}
              <blockquote className="relative border-l-2 border-nsdc-blue/50 pl-6 mb-12">
              ...
              >
                <div className="flex flex-col md:flex-row items-center gap-6 text-center md:text-left">
                  <div className="w-20 h-20 md:w-24 md:h-24 rounded-full bg-gradient-to-br from-nsdc-blue/40 to-nsdc-blue/10 border-2 border-white/10 flex items-center justify-center flex-shrink-0">
                    <span className="text-2xl md:text-3xl font-bold text-nsdc-blue">
                      {post.author.name.split(' ').map(n => n[0]).join('')}
                    </span>
                  </div>
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-nsdc-blue mb-3">
              ...
              <div className="mt-16 pt-12 border-t border-white/5">
                ...
                <button
                  onClick={onClose}
                  className="group inline-flex items-center gap-3 text-nsdc-blue hover:text-[var(--foreground)] transition-colors"
                >
          ...
          <AnimatePresence>
            {showScrollTop && (
              <motion.button
                ...
                whileTap={{ scale: 0.9 }}
                onClick={scrollToTop}
                className="fixed bottom-6 right-6 md:bottom-8 md:right-8 z-50 w-12 h-12 rounded-full bg-nsdc-blue text-[var(--foreground)] flex items-center justify-center shadow-lg shadow-nsdc-blue/20"
              >
