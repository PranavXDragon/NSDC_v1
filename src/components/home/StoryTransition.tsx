...
export default function StoryTransition({
  ...
  const renderMainText = () => {
    ...
    return parts.map((part, i) => {
      if (part.toLowerCase() === accentWord.toLowerCase()) {
        return (
          <span key={i} className="text-nsdc-blue">
  ...
  return (
    ...
    >
      ...
      <motion.div
        style={{ y: y2, opacity }}
        className="absolute bottom-1/4 right-10 w-48 h-48 border border-nsdc-blue/10 rounded-full"
      />

      {/* Ambient Glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[400px] bg-nsdc-blue/3 rounded-full blur-[150px] pointer-events-none" />
