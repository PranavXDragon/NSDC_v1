...
function MemberCard({ member, index }: { member: TeamMember; index: number }) {
  ...
  return (
    ...
    >
      {/* Card Container */}
      <div className="relative h-full flex flex-col bg-[#0a0a0f] border border-white/6 overflow-hidden transition-all duration-500 hover:border-white/12">
        {/* Corner Accents */}
        <div className="absolute top-0 left-0 w-4 h-4 border-l-2 border-t-2 border-nsdc-blue/60" />
        <div className="absolute top-0 right-0 w-4 h-4 border-r-2 border-t-2 border-nsdc-blue/60" />
        <div className="absolute bottom-0 left-0 w-4 h-4 border-l-2 border-b-2 border-nsdc-blue/60" />
        <div className="absolute bottom-0 right-0 w-4 h-4 border-r-2 border-b-2 border-nsdc-blue/60" />

        {/* Image Container */}
        <div className="relative aspect-4/5 shrink-0 overflow-hidden bg-linear-to-br from-white/2 to-transparent">
          {member.imageUrl ? (
            <Image
              src={member.imageUrl}
              alt={member.name}
        ...
        {/* Content */}
        <div className="relative p-5 -mt-12 flex-1">
          <h3
          ...
          {member.linkedin && (
            <a
              ...
              className="absolute bottom-5 right-5 w-8 h-8 flex items-center justify-center bg-white/5 border border-white/10 text-white/40 hover:text-nsdc-blue hover:border-nsdc-blue/40 transition-all duration-300"
            >
              <Linkedin size={14} />
            </a>
          )}
    ...
    >
      <div>
        <div className="flex items-center gap-4 mb-3">
          <span
            className="text-nsdc-blue text-xs font-medium tracking-[0.3em]"
            style={{ fontFamily: "var(--font-body)" }}
          >
            {index}
          </span>
        ...
        <div className="max-w-[1400px] mx-auto">
          ...
          >
            <div className="flex items-center gap-6 mb-6">
              <div className="w-16 h-px bg-linear-to-r from-nsdc-blue/60 to-transparent" />
              <span
            ...
            <p
              ...
              style={{ fontFamily: "var(--font-body)" }}
            >
              Meet the people building and leading the NSDC SCET community.
            </p>
          </motion.div>
          ...
          >
            <Link
              href="/teams"
              className="group inline-flex items-center gap-4 px-8 py-4 bg-white/3 border border-white/10 hover:border-nsdc-blue/40 hover:bg-nsdc-blue/5 transition-all duration-500"
            >
              <span
              ...
              <ArrowRight
                size={16}
                className="text-white/40 group-hover:text-nsdc-blue group-hover:translate-x-1 transition-all duration-300"
              />
            </Link>
          </motion.div>
        </div>
